...
//Clear answer structure
  ...
  /*if (!checkExist || !m_memoryCtx.HelperFindBySystemIdtf(formItems[3], checkNode))
  {
    ScAddr error;
    ...
    m_memoryCtx.HelperSetSystemIdtf("_node_not_exist", newError);
    m_memoryCtx.CreateEdge(ScType::EdgeAccessVarPosPerm, error, newError);
    SC_LOG_DEBUG("CreateClassAgent finished with error: nodes not exist");
    utils::AgentUtils::finishAgentWork(&m_memoryCtx, questionNode, false);
    return SC_RESULT_OK;
  }*/

  if (formItems[0] == "concept_" || formItems[1] == "" || formItems[2] == "" || formItems[3] == "concept_" || checkInvalid)
  {
...
//Create Class Construction
  ...
  ScAddr const & superClassNode = m_memoryCtx.CreateNode(ScType::NodeConstClass);
  m_memoryCtx.HelperSetSystemIdtf(formItems[3], superClassNode);
  ...
  if(formItems.size()>=4){
    ...
    for (size_t i=4; i<formItems.size(); i++){
      ScAddr const & subdividingClassNode = m_memoryCtx.CreateNode(ScType::NodeConstClass);
      m_memoryCtx.HelperSetSystemIdtf(formItems[i], subdividingClassNode);
      
      classConstruction.Triple(
        "tuple",
        ScType::EdgeAccessVarPosPerm,
        subdividingClassNode
      );
    }
  }
